...
static ps3_in_report_alt_t ps3_report_n[USB_OUTPUT_PADS];
#else
static ps3_in_report_t ps3_report_n[USB_OUTPUT_PADS];
#endif
static ps3_out_report_t ps3_output_n[USB_OUTPUT_PADS];
static bool ps3_output_available_n[USB_OUTPUT_PADS] = {false};
static uint8_t ps3_ef_byte_n[USB_OUTPUT_PADS] = {0};  // Mode byte echoed from SET_REPORT(0xEF)
static ps3_pairing_info_t ps3_pairing_n[USB_OUTPUT_PADS] = {0};  // Pairing info with BT addresses
...
static void ps3_mode_init(void)
{
    ...
    {
        printf("ps3_mode_init(%d)\n", i);
#if USB_OUTPUT_PADS > 1
        ps3_init_report_alt(&ps3_report_n[i]);
#else
        ps3_init_report(&ps3_report_n[i]);
#endif
        memset(&ps3_output_n[i], 0, sizeof(ps3_out_report_t));
        ps3_output_available_n[i] = false;
        ps3_ef_byte_n[i] = 0;

        // Generate plausible BT addresses from board unique ID
        memset(&ps3_pairing_n[i], 0, sizeof(ps3_pairing_n[i]));
        uint8_t board_id[8];
        platform_get_unique_id(board_id, sizeof(board_id));
        // Device address: bytes 0-5 of board ID
        ps3_pairing_n[i].device_address[0] = 0x00;  // Leading zero
        for (int j = 0; j < 6; j++) {
            ps3_pairing_n[i].device_address[1 + j] = board_id[j];
        }
        ps3_pairing_n[i].device_address[6] += i;

        // Host address: bytes 2-7 XOR'd for differentiation
        ps3_pairing_n[i].host_address[0] = 0x00;  // Leading zero
        for (int j = 0; j < 6; j++) {
            ps3_pairing_n[i].host_address[1 + j] = board_id[j] ^ 0xAA;
        }
        // ps3_pairing_n[i].host_address[6] ^= i;
    }
}
...
static bool ps3_mode_is_ready_itf(uint8_t itf)
{
...
{
#if USB_OUTPUT_PADS > 1 
    ps3_in_report_alt_t* ps3_report = &ps3_report_n[player_index];
    // Digital buttons byte 0
    ps3_report->buttons[0] = 0;
    if (buttons & JP_BUTTON_B3) ps3_report->buttons[0] |= 0x01; // Square / West
    if (buttons & JP_BUTTON_B1) ps3_report->buttons[0] |= 0x02; // Cross / South
    if (buttons & JP_BUTTON_B2) ps3_report->buttons[0] |= 0x04; // Circle / East
    if (buttons & JP_BUTTON_B4) ps3_report->buttons[0] |= 0x08; // Triangle / North
    if (buttons & JP_BUTTON_L1) ps3_report->buttons[0] |= 0x10;
    if (buttons & JP_BUTTON_R1) ps3_report->buttons[0] |= 0x20;
    if (buttons & JP_BUTTON_L2) ps3_report->buttons[0] |= 0x40;
    if (buttons & JP_BUTTON_R2) ps3_report->buttons[0] |= 0x80;

    // Digital buttons byte 1
    ps3_report->buttons[1] = 0;
    if (buttons & JP_BUTTON_S1) ps3_report->buttons[1] |= 0x01; // Select
    if (buttons & JP_BUTTON_S2) ps3_report->buttons[1] |= 0x02; // Start
    if (buttons & JP_BUTTON_L3) ps3_report->buttons[1] |= 0x04;
    if (buttons & JP_BUTTON_R3) ps3_report->buttons[1] |= 0x08;
    if (buttons & JP_BUTTON_A1) ps3_report->buttons[1] |= 0x10; // PS

    // Map D-Pad Hat Switch byte 2
    uint32_t dpad = buttons & (JP_BUTTON_DU | JP_BUTTON_DD | JP_BUTTON_DL | JP_BUTTON_DR);
    switch (dpad) {
        case JP_BUTTON_DU:                               ps3_report->buttons[2] = PS3_HAT_UP;        break;
        case JP_BUTTON_DU | JP_BUTTON_DR:                ps3_report->buttons[2] = PS3_HAT_UPRIGHT;   break;
        case JP_BUTTON_DR:                               ps3_report->buttons[2] = PS3_HAT_RIGHT;     break;
        case JP_BUTTON_DD | JP_BUTTON_DR:                ps3_report->buttons[2] = PS3_HAT_DOWNRIGHT; break;
        case JP_BUTTON_DD:                               ps3_report->buttons[2] = PS3_HAT_DOWN;      break;
        case JP_BUTTON_DD | JP_BUTTON_DL:                ps3_report->buttons[2] = PS3_HAT_DOWNLEFT;  break;
        case JP_BUTTON_DL:                               ps3_report->buttons[2] = PS3_HAT_LEFT;      break;
        case JP_BUTTON_DU | JP_BUTTON_DL:                ps3_report->buttons[2] = PS3_HAT_UPLEFT;    break;
        default:                                         ps3_report->buttons[2] = PS3_HAT_NOTHING;   break;
    }
    ...
    bool result = tud_hid_n_report(player_index, 0, ps3_report, sizeof(ps3_in_report_alt_t));
#else
    ps3_in_report_t* ps3_report = &ps3_report_n[player_index];
    // Digital buttons byte 0
    ps3_report->buttons[0] = 0;
    ...
    return tud_hid_n_report(player_index, 0, ps3_report, sizeof(ps3_in_report_t));
#endif
}

static void ps3_mode_handle_output_itf(uint8_t itf, uint8_t report_id, const uint8_t* data, uint16_t len)
{
    (void)report_id;
    ...
    if (len >= sizeof(ps3_out_report_t)) {
        memcpy(&ps3_output_n[itf], data, sizeof(ps3_out_report_t));
        ps3_output_available_n[itf] = true;
    }
}
...
static uint8_t ps3_mode_get_rumble(void)
{
    ...
    return (ps3_output_n[0].rumble_left_force > 0) ? ps3_output_n[0].rumble_left_force :
           (ps3_output_n[0].rumble_right_on > 0) ? 0xFF : 0x00;
}

static bool ps3_mode_get_feedback(output_feedback_t* fb)
{
    if (!ps3_output_available_n[0]) return false;

    // PS3: left is variable force, right is on/off only
    fb->rumble_left = ps3_output_n[0].rumble_left_force;
    fb->rumble_right = ps3_output_n[0].rumble_right_on ? 0xFF : 0x00;

    // PS3 LEDs: bitmap in leds_bitmap, shifted by 1 (LED1=0x02..LED4=0x10)
    // Combinations encode players 5-7 via PLAYER_LEDS[] lookup
    uint8_t led_bits = (ps3_output_n[0].leds_bitmap >> 1) & 0x0F;
    fb->led_player = 0;
    for (int p = 1; p <= 7; p++) {
    ...
    fb->dirty = true;
    ps3_output_available_n[0] = false;
    return true;
}
...
void ps3_mode_set_feature_report(uint8_t itf, uint8_t report_id, const uint8_t* buffer, uint16_t bufsize)
{
    if (report_id == PS3_REPORT_ID_FEATURE_EF && bufsize > 6) {
        ps3_ef_byte_n[itf] = buffer[6];
    }
}

static uint16_t ps3_mode_get_report_itf(uint8_t itf, uint8_t report_id, hid_report_type_t report_type,
                                     uint8_t* buffer, uint16_t reqlen)
{
    ...
    uint16_t len = 0;
    switch (report_id) {
        case PS3_REPORT_ID_FEATURE_00: 
            len = sizeof(ps3_feature_00);
            if (reqlen < len) len = reqlen;
            memcpy(buffer, ps3_feature_00, len);
            return len;
        ...
        case PS3_REPORT_ID_PAIRING:
            len = sizeof(ps3_pairing_n[itf]);
            if (reqlen < len) len = reqlen;
            memcpy(buffer, &ps3_pairing_n[itf], len);
            return len;
        ...
        case PS3_REPORT_ID_FEATURE_EF:
            ...
            buffer[6] = ps3_ef_byte_n[itf];  // Echo mode byte from SET_REPORT
            return len;
        ...
        case PS3_REPORT_ID_FEATURE_F5:
            ...
            for (int i = 0; i < 6; i++) {
                buffer[1 + i] = ps3_pairing_n[itf].host_address[1 + i];
            }
            return len;
...
{
    printf("ps3_mode_get_report_descriptor\n");
#if (USB_OUTPUT_PADS > 1)
...
const usbd_mode_t ps3_mode = {
    ...
    .is_ready_itf = ps3_mode_is_ready_itf,

    .handle_output_itf = ps3_mode_handle_output_itf,
    .get_rumble = ps3_mode_get_rumble,
    .get_feedback = ps3_mode_get_feedback,
    .get_report_itf = ps3_mode_get_report_itf,
